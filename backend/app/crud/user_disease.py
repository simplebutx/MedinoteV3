...
def get_user_disease(db: Session, user_id: int) -> list[UserDisease]:
    ...
    try:
        ...
        db.rollback()
        raise DatabaseError("기저질환 목록 조회 중 오류가 발생했어요.") from error
...
def create_user_disease(
    ...
    if disease_code:
        ...
        try:
            ...
            db.rollback()
            raise DatabaseError("질병 코드 확인 중 오류가 발생했어요.") from error

        if disease_master is None:
            raise BadRequestError("존재하지 않는 질병 코드예요.")
    ...
    try:
        ...
        db.rollback()
        raise BadRequestError("등록할 수 없는 질병 정보예요.") from error

    except SQLAlchemyError as error:
        db.rollback()
        raise DatabaseError("기저질환 저장 중 오류가 발생했어요.") from error
...
def delete_user_disease(
    ...
    if user_disease is None:
        raise NotFoundError("기저질환을 찾을 수 없어요.")
    ...
    try:
        ...
        db.rollback()
        raise DatabaseError("기저질환 삭제 중 오류가 발생했어요.") from error
...
def get_disease_names(
    ...
    try:
        ...
        db.rollback()
        raise DatabaseError("기저질환 자동완성 조회 중 오류가 발생했어요.") from error
